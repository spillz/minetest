...
def read_map_data(args,cur,map_info,str_to_uid,xlist,zlist,minx,maxx,minz,maxz):
    read_map_time = 0
    ...
    #                mapdata2 = numpy.array([])
                ...
                if len(plist) == 0 or ypos==ylist[0]:
                    ...
                    map_info['height'][pos] = hdata.reshape(16,16)
                    map_info['content'][pos] = cdata.reshape(16,16)
                    map_info['water'][pos] = wdata.reshape(16,16)
                    map_info['dnd'][pos] = dnddata.reshape(16,16)
                    break
...
def draw_image(args,stuff,uid_to_color,w,h,minx,minz,maxx,maxz):
    print("Drawing image")
    # Drawing the picture
    starttime = time.time()
...
def main():
    ...
    cur, xlist, zlist = generate_sector_list(args)
    ...
    read_map_data(args,cur,map_info,str_to_uid,xlist,zlist,minx,maxx,minz,maxz)

    draw_image(args,map_info,uid_to_color,w,h,minx,minz,maxx,maxz)
